...
const BADGES = [
  { id: 'first_report', label: 'Premier Signalement', desc: 'Avoir soumis votre 1er signalement', points: 10, threshold: 1 },
  { id: 'contributor_5', label: 'Contributeur', desc: '5 signalements validés', points: 25, threshold: 5 },
  { id: 'vigilant_20', label: 'Sentinelle', desc: '20 signalements validés', points: 100, threshold: 20 },
  { id: 'hero_50', label: 'Citoyen Actif', desc: '50 signalements validés', points: 250, threshold: 50 },
  { id: 'champion_100', label: 'Champion Citoyen', desc: '100 signalements validés', points: 500, threshold: 100 },
  { id: 'confirmed', label: 'Vérificateur', desc: '10 confirmations communautaires', points: 50, threshold: 10 },
  { id: 'photo', label: 'Preuve Visuelle', desc: '5 signalements avec photo', points: 30, threshold: 5 },
  { id: 'critical', label: "Détecteur d'Urgence", desc: '3 alertes critiques confirmées', points: 75, threshold: 3 },
]

const LEVELS = [
  { name: 'Observateur', min: 0, max: 49, color: 'text-gray-600', bg: 'bg-gray-100' },
  { name: 'Signaleur', min: 50, max: 149, color: 'text-blue-600', bg: 'bg-blue-100' },
  { name: 'Contributeur', min: 150, max: 399, color: 'text-green-600', bg: 'bg-green-100' },
  { name: 'Sentinelle', min: 400, max: 999, color: 'text-purple-600', bg: 'bg-purple-100' },
  { name: 'Champion Citoyen', min: 1000, max: Infinity, color: 'text-yellow-600', bg: 'bg-yellow-100' },
]
...
export default function Gamification() {
  ...
  useEffect(() => {
    ...
    api.get('/consumer/leaderboard').then(r => setLeaderboard(r.data)).catch(() => setLeaderboard([
      { rank: 1, name: 'Aminata D.', points: 1250, reports: 128 },
      { rank: 2, name: 'Moussa K.', points: 980, reports: 97 },
      { rank: 3, name: 'Fatou S.', points: 720, reports: 74 },
      { rank: 4, name: 'Ibrahim T.', points: 610, reports: 61 },
      { rank: 5, name: 'Aissatou B.', points: 530, reports: 53 },
      { rank: 6, name: 'Cheikh N.', points: 420, reports: 42 },
      { rank: 7, name: 'Vous', points: 185, reports: 23, isMe: true },
      { rank: 8, name: 'Rokhaya L.', points: 150, reports: 15 },
      { rank: 9, name: 'Bamba F.', points: 120, reports: 12 },
      { rank: 10, name: 'Oumou D.', points: 95, reports: 9 },
    ]))
  }, [])
      ...
      {tab === 'profile' && profile && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Signalements', value: profile.reportsCount, sub: `${profile.validatedCount} validés` },
              { label: 'Confirmations', value: profile.confirmationsGiven, sub: 'données' },
              { label: 'Avec photos', value: profile.photosCount, sub: 'preuves visuelles' },
              { label: 'Alertes critiques', value: profile.criticalAlerts, sub: 'confirmées' },
            ].map(({ label, value, sub }) => (
            ...
            {leaderboard.map((u, i) => (
              <div key={i} className={`flex items-center gap-4 px-4 py-3 ${u.isMe ? 'bg-blue-50 border-l-4 border-blue-500' : 'hover:bg-gray-50'}`}>
                <div className="w-8 text-center">
                  {u.rank <= 3 ? <span className="text-lg font-black text-yellow-600">#{u.rank}</span> : <span className="text-gray-400 font-bold">#{u.rank}</span>}
                </div>
