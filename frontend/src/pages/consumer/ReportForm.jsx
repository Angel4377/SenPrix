...
const BADGE_THRESHOLDS = [
  { id: 'first_report',   label: 'Premier Signalement', count: 1 },
  { id: 'contributor_5',  label: 'Contributeur',         count: 5 },
  { id: 'vigilant_20',    label: 'Sentinelle',           count: 20 },
  { id: 'hero_50',        label: 'Citoyen Actif',        count: 50 },
  { id: 'champion_100',   label: 'Champion Citoyen',     count: 100 },
]
...
function StepBar({ step }) {
  const steps = [
    { n: 1, label: 'Produit & Prix' },
    { n: 2, label: 'Lieu & Preuves' },
            ...
            <div className={`rounded-lg px-4 py-3 border ${
              ...
              {ecart !== null && (
                <p className={`text-xs font-bold mt-0.5 ${ecartColor}`}>
                  {+ecart > 0 ? ` Écart : +${ecart}%` : ` En dessous du plafond (${ecart}%)`}
                </p>
              )}
            </div>
          ) : (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
        ...
        {form.photos.length > 0 && (
          <div className="flex items-center gap-2 text-xs text-blue-600 bg-blue-50 rounded-lg px-3 py-2">
            +5 points bonus pour signalement avec photo !
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
         Ce signalement sera traité par les services compétents. Vos données personnelles ne seront pas divulguées aux commerçants.
      </div>
...
function SuccessScreen({ result, form, products, regions, navigate }) {
  ...
  const priorityMap = {
    ...
    HIGH:     { label: 'ÉLEVÉE',     color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' },
    NORMAL:   { label: 'NORMALE',    color: 'text-blue-600',   bg: 'bg-blue-50',   border: 'border-blue-200' },
    LOW:      { label: 'BASSE',      color: 'text-green-600',  bg: 'bg-green-50',  border: 'border-green-200' },
  }
    ...
    <div className="max-w-xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-8 text-center">
        ...
        <div className="mt-4 inline-flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2 text-sm text-gray-600">
          Référence : <strong>#{result.id}</strong>
        </div>
      </div>
